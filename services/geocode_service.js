...
=====================================================
GEOCODE SERVICE
Reverse-geocodes lat/lng to a pincode using
Google Maps Geocoding API.
No changes from V2 — the existing implementation
is correct and handles Indian pincodes reliably.
=====================================================
*/
...
const extractPincodeFromComponents = (components = []) => {
  const postal = components.find(
    (item) =>
      Array.isArray(item?.types) && item.types.includes("postal_code")
  );
  return normalizePincode(postal?.long_name || postal?.short_name || "");
};

const extractPincodeFromResult = (result) => {
...
const reverseGeocode = async (latitude, longitude) => {
  ...
  if (!first) {
    return {
      ...
      address: "",
      google: {
        status: googleStatus || "EMPTY_RESULTS",
        errorMessage: googleErrorMessage || "",
      },
    };
  }
